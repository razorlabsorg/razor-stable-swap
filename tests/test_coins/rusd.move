#[test_only]
module razor_stable_swap::rusd_token {
    ...
    const ASSET_SYMBOL: vector<u8> = b"RUSD";
    const ASSET_NAME: vector<u8> = b"RAZOR USD";
    const DECIMALS: u8 = 8;
    ...
    fun owner(): address acquires Ownership {
        let owner = borrow_global<Ownership>(token_address()).owner;
        return owner
    }

    #[view]
    public fun token_address(): address {
        object::create_object_address(&@razor_stable_swap, ASSET_SYMBOL)
    }
    ...
    fun init_module(admin: &signer) {
        ...
        primary_fungible_store::create_primary_store_enabled_fungible_asset(
            ...
            utf8(ASSET_SYMBOL),
            DECIMALS,
            utf8(b"https://ipfs.io/ipfs/QmYbAuxRGdSgNsfDopufzRrXsXfeuRsMnd1T1JR7qdi5Kn"),
    ...
    public entry fun set_pause(admin: &signer, paused: bool) acquires Ownership, State {
        let ownership = borrow_global<Ownership>(token_address());
    ...
    public entry fun transfer_ownership(admin: &signer, new_owner: address) acquires Ownership, State {
        assert_not_paused();
        let ownership = borrow_global_mut<Ownership>(token_address());
        assert!(signer::address_of(admin) == ownership.owner, ERROR_UNAUTHORIZED);
