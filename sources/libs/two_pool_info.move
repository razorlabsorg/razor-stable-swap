module razor_stable_swap::two_pool_info {
    use std::vector;
    use std::option;
    ...
    #[view]
    public fun rates(pool: Object<TwoPool>): vector<u256> {
        two_pool::pool_rates(&pool)
    }
    ...
    #[view]
    public fun balances_with_positions(pool: Object<TwoPool>, positions: vector<address>): vector<u256> {
    ...
    #[view]
    public fun precision_mul(pool: Object<TwoPool>): vector<u256> {
        let precision_muls = vector::empty<u256>();
        let (token0, token1) = two_pool::unpack_pool(pool);
        let tokens = vector[token0, token1];
        let i = 0;
        while (i < N_COINS) {
    ...
    #[view]
    public fun get_d_mem(pool: Object<TwoPool>, balances: vector<u256>, amp: u256): u256 {
        let xp = xp_mem(balances, pool);
        two_pool::get_d(&xp, amp)
    }
    ...
    fun xp_mem(balances: vector<u256>, pool: Object<TwoPool>): vector<u256> {
    ...
    #[view]
    public fun get_add_liquidity_fee(pool: Object<TwoPool>, amounts: vector<u256>): vector<u256> {
        let pool_fee = two_pool::pool_fee(&pool);
        let fee = (pool_fee * (N_COINS as u256)) / (4 * ((N_COINS - 1) as u256));
    ...
    #[view]
    public fun get_remove_liquidity_imbalance_fee(pool: Object<TwoPool>, amounts: vector<u256>): vector<u256> {
        let pool_fee = two_pool::pool_fee(&pool);
        let fee = (pool_fee * (N_COINS as u256)) / (4 * ((N_COINS - 1) as u256));
        let admin_fee = two_pool::pool_admin_fee(&pool);
        let amp = two_pool::a(pool);
