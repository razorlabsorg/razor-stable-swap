module razor_stable_swap::three_pool {
    ...
    public(friend) fun initialize(
        ...
        let pool_constructor_ref = create_lp_token(token0, token1, token2);
        ...
        while (i < N_COINS) {
            ...
            assert!(coin_decimals <= MAX_COIN_DECIMAL, swap_errors::coin_decimal_too_high());

            vector::push_back(&mut balances, 0);
            let precision_mul = pow(10, (MAX_COIN_DECIMAL - coin_decimals as u64));
        ...
        // Move the ThreePool resource to the pool object
        move_to(&pool_signer, ThreePool {
            coins: coin_stores,
    ...
    public(friend) fun a(pool: &Object<ThreePool>): u256 acquires ThreePool {
        let pool_data = pool_data<ThreePool>(pool);
    ...
    fun xp(pool: &ThreePool): vector<u256> {
    ...
    fun xp_mem(balances: &vector<u256>, pool: &ThreePool): vector<u256> {
        let result = vector::empty<u256>();
        let i = 0;
        while (i < N_COINS) {
    ...
    fun get_d_mem(balances: &vector<u256>, amp: u256, pool: &ThreePool): u256 {
        let xp = xp_mem(balances, pool);
        get_d(&xp, amp)
    }

    #[view]
    public fun get_virtual_price(pool: Object<ThreePool>): u256 acquires ThreePool {
        let pool_data = pool_data<ThreePool>(&pool);
    ...
    public(friend) fun calc_token_amount(
        ...
        let pool = stable_swap_pool(token0, token1, token2);
        let total_supply_opt = fungible_asset::supply(pool);
        let total_supply = (option::extract(&mut total_supply_opt) as u256);
    ...
    public(friend) fun add_liquidity(
        ...
        sender: &signer
    ): FungibleAsset acquires ThreePool {
        ...
        if (!is_sorted(token0, token1, token2)) {
            ...
            vector::push_back(&mut sortedAmounts, vector::remove(&mut amounts, (*vector::borrow(&tokenPositions, 0) as u64)));
            vector::push_back(&mut sortedAmounts, vector::remove(&mut amounts, (*vector::borrow(&tokenPositions, 0) as u64)));
            vector::push_back(&mut sortedAmounts, vector::pop_back(&mut amounts)); // Last element

            // Now amounts should be empty
            vector::destroy_empty(amounts);
            
            return add_liquidity(pool, sortedAmounts, min_mint_amount, sender)
        };
        ...
        for (i in 0..N_COINS) {
            ...
            dispatchable_fungible_asset::deposit(*vector::borrow(&pool_data.coins, i), coin)
        };
    ...
    fun get_y(i: u64, j: u64, x: u256, xp: &vector<u256>, pool: &ThreePool): u256 {
    ...
    public(friend) fun get_dy(i: u64, j: u64, dx: u256, pool: &Object<ThreePool>): u256 acquires ThreePool {
        let pool_data = pool_data<ThreePool>(pool);
    ...
    public(friend) fun get_dy_underlying(
        i: u64,
        j: u64,
        dx: u256,
        pool: &Object<ThreePool>
    ...
    public(friend) fun remove_liquidity(
        ...
        while (i < N_COINS) {
            ...
            *vector::borrow_mut(balances, i) = *vector::borrow(balances, i) - amount;
            ...
            let asset = dispatchable_fungible_asset::withdraw(pool_signer, token_store, (amount as u64));
    ...
    public(friend) fun calc_withdraw_one_coin(
        pool: &Object<ThreePool>,
        token_amount: u256,
        i: u64,
    ): (u256, u256) acquires ThreePool {
    ...
    public(friend) fun ramp_a(admin: &signer, future_a: u256, future_time: u256, pool: &Object<ThreePool>) acquires ThreePool {
    ...
    public(friend) fun stop_rampget_a(admin: &signer, pool: &Object<ThreePool>) acquires ThreePool {
    ...
    public(friend) fun revert_new_parameters(admin: &signer, pool: &Object<ThreePool>) acquires ThreePool {
    ...
    public(friend) fun pool_coins<T: key>(pool: &Object<T>): vector<Object<FungibleStore>> acquires ThreePool {
       pool_data(pool).coins
    }

    public(friend) fun pool_fee<T: key>(pool: &Object<T>): u256 acquires ThreePool {
        pool_data(pool).fee
    }

    public(friend) fun pool_admin_fee<T: key>(pool: &Object<T>): u256 acquires ThreePool {
        pool_data(pool).admin_fee
    }

    public(friend) fun pool_initial_a<T: key>(pool: &Object<T>): u256 acquires ThreePool {
        pool_data(pool).initial_a
    }

    public(friend) fun pool_future_a<T: key>(pool: &Object<T>): u256 acquires ThreePool {
        pool_data(pool).future_a
    }

    public(friend) fun pool_initial_a_time<T: key>(pool: &Object<T>): u64 acquires ThreePool {
        pool_data(pool).initial_a_time
    }

    public(friend) fun pool_future_a_time<T: key>(pool: &Object<T>): u64 acquires ThreePool {
        pool_data(pool).future_a_time
    }

    public(friend) fun pool_kill_deadline<T: key>(pool: &Object<T>): u64 acquires ThreePool {
        pool_data(pool).kill_deadline
    }

    public(friend) fun pool_admin_actions_deadline<T: key>(pool: &Object<T>): u64 acquires ThreePool {
        pool_data(pool).admin_actions_deadline
    }

    public(friend) fun pool_future_fee<T: key>(pool: &Object<T>): u256 acquires ThreePool {
        pool_data(pool).future_fee
    }

    public(friend) fun pool_future_admin_fee<T: key>(pool: &Object<T>): u256 acquires ThreePool {
        pool_data(pool).future_admin_fee
    }

    public(friend) fun pool_is_killed<T: key>(pool: &Object<T>): bool acquires ThreePool {
        pool_data(pool).is_killed
    }

    public(friend) fun pool_tokens<T: key>(pool: &Object<T>): (Object<Metadata>, Object<Metadata>, Object<Metadata>) acquires ThreePool {
        let pool_data = pool_data(pool);
        (pool_data.token0, pool_data.token1, pool_data.token2)
    }

    public(friend) fun pool_balances<T: key>(pool: &Object<T>): vector<u256> acquires ThreePool {
        pool_data(pool).balances
    }

    public(friend) fun pool_precision_muls<T: key>(pool: &Object<T>): vector<u256> acquires ThreePool {
        pool_data(pool).precision_muls
    }

    public(friend) fun pool_rates<T: key>(pool: &Object<T>): vector<u256> acquires ThreePool {
        pool_data(pool).rates
    }

    inline fun pool_data<T: key>(pool: &Object<T>): &ThreePool acquires ThreePool {
        borrow_global<ThreePool>(object::object_address(pool))
    }

    inline fun pool_data_mut<T: key>(pool: &Object<T>): &mut ThreePool acquires ThreePool {
    ...
    inline fun create_lp_token(
        ...
        let token_name = lp_token_name(token0, token1, token2);
        let seeds = get_pool_seeds(token0, token1, token2);
    ...
    inline fun lp_token_name(token0: Object<Metadata>, token1: Object<Metadata> ,token2: Object<Metadata>): String {
        ...
        string::append_utf8(&mut token_symbol, b"-");
        string::append(&mut token_symbol, fungible_asset::symbol(token2));
        string::append_utf8(&mut token_symbol, b" LP");
        token_symbol
    }

    public fun get_pool_seeds(token0: Object<Metadata>, token1: Object<Metadata>, token2: Object<Metadata>): vector<u8> {
        let seeds = vector[];
    ...
    #[view]
    public(friend) fun unpack_pool(pool: Object<ThreePool>): (Object<Metadata>, Object<Metadata>, Object<Metadata>) acquires ThreePool {
        let pool = pool_data(&pool);
        (pool.token0, pool.token1, pool.token2)
    }
    ...
    public fun stable_swap_pool(
        ...
        object::address_to_object(pool_address(token0, token1, token2))
    }
    ...
    public fun stable_swap_pool_address_safe(
        ...
        let pool_address = pool_address(token0, token1, token2);
        (exists<ThreePool>(pool_address), pool_address)
    }
